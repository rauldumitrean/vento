...
const Sidebar = ({ view, setView, handleLogout, userName, isPremium, onNewConsulta, token }) => {
  return (
    <div className="hidden lg:flex flex-col w-[280px] h-full bg-black/40 border border-white/10 rounded-[2rem] shadow-2xl relative shrink-0 z-20 overflow-hidden">
      {/* Decorative top glow */}
      ...
      {/* ── FIXED HEADER ── */}
      <div className="shrink-0 p-6 pb-0 relative z-10">
        {/* Logo + Bell */}
        ...
        <button 
          ...
          className="w-full mb-4 flex items-center gap-3 px-4 py-3.5 bg-white/10 hover:bg-white/15 border border-white/10 rounded-xl text-white font-medium transition-all duration-300 shadow-md group relative overflow-hidden"
        >
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar px-6 py-2 relative z-10">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2 pl-2 mt-4">Social</p>
          <SidebarItem icon={User} label="Amigos y Chat" isActive={view === 'friends'} onClick={() => setView('friends')} />
        </div>
      </div>

      {/* ── FIXED FOOTER ── */}
      <div className="shrink-0 px-6 pb-6 pt-2 relative z-10">
        ...
        <div className="pt-3 border-t border-white/5 flex items-center justify-between gap-3">
          <button onClick={() => setView('profile')} className="flex items-center gap-3 hover:bg-white/5 p-2 -ml-2 rounded-xl transition-colors flex-1 min-w-0">
