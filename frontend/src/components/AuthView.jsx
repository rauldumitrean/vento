...
const CustomGoogleButton = ({ onSuccess, onError }) => {
  ...
  return (
    <button
      type="button"
      onClick={() => login()}
      className="w-full flex items-center justify-center gap-3 bg-white border border-gray-200 hover:bg-gray-50 hover:border-gray-300 text-gray-700 font-medium py-3 px-4 rounded-xl shadow-sm transition-all duration-200"
    >
...
export default function AuthView({ setToken }) {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      const payload = {
        ...
        gender: isLogin ? null : gender,
        age: isLogin ? null : age,
      };
  ...
  const submitPendingGoogleAuth = async () => {
    if (!gender || !age) return setError('Por favor completa tu género y edad.');
    setError('');
    setLoading(true);
    try {
      const payload = {
